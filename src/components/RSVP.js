...
function RSVP({
  ...
  const [formData, setFormData] = useState({ name: '', email: '', attendance: '', guests: '1', menu: '', allergies: '', message: '' });
      ...
      <Section ref={sectionRef} id="rsvp">
        <Container>
          <SuccessMessage>
            ...
            <h3>Wunderbar!</h3>
            <p>{formData.attendance === 'yes' ? 'Wir freuen uns riesig auf euch!' : 'Schade – ihr werdet uns fehlen.'}</p>
          </SuccessMessage>
        </Container>
      </Section>
    );
    ...
    <Section ref={sectionRef} id="rsvp">
      ...
      <Container>
        ...
        <Form visible={visible} onSubmit={handleSubmit}>
          <FormGroup>
            <Label>Name</Label>
            <Input type="text" placeholder="Euer Name" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} required />
          </FormGroup>
          
          <FormGroup>
            <Label>E-Mail</Label>
            <Input type="email" placeholder="eure@email.de" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} required />
          </FormGroup>
          
          <FormGroup>
            <Label>Kommt ihr?</Label>
            <RadioGroup>
              <RadioLabel><input type="radio" name="attendance" value="yes" checked={formData.attendance === 'yes'} onChange={(e) => setFormData({...formData, attendance: e.target.value})} required /> Ja, wir kommen!</RadioLabel>
              <RadioLabel><input type="radio" name="attendance" value="no" checked={formData.attendance === 'no'} onChange={(e) => setFormData({...formData, attendance: e.target.value})} /> Leider nicht</RadioLabel>
            </RadioGroup>
          </FormGroup>
          ...
          {formData.attendance === 'yes' && (
            <>
              ...
              <FormGroup>
                ...
                <Select value={formData.menu} onChange={(e) => setFormData({...formData, menu: e.target.value})} required>
                  <option value="">Bitte wählen</option>
                  {menuOptions.map((opt, i) => <option key={i} value={opt.toLowerCase()}>{opt}</option>)}
                </Select>
              </FormGroup>
              
              <FormGroup>
                <Label>Allergien (optional)</Label>
                <Input type="text" placeholder="z.B. Nüsse, Laktose" value={formData.allergies} onChange={(e) => setFormData({...formData, allergies: e.target.value})} />
              </FormGroup>
            </>
          )}
          ...
          <FormGroup>
            ...
            <Textarea placeholder="Möchtet ihr uns etwas mitteilen?" value={formData.message} onChange={(e) => setFormData({...formData, message: e.target.value})} />
          </FormGroup>
          
          <SubmitBtn type="submit">Antwort senden</SubmitBtn>
        </Form>
